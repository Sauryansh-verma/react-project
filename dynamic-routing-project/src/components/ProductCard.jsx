...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      {/* Product Image */}
      <div onClick={() => navigate(`/details/${product.id}`)} className="flex h-72 items-center justify-center bg-gray-50 p-6">
        <img
          src={product.image}
          alt={product.title}
      ...
      <div className="p-5">
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          <button className="rounded-lg bg-gray-900 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-blue-600 active:scale-95">
            Add to Cart
          </button>
        </div>
      </div>
